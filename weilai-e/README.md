...
> 「未来鹅」= 你的腾讯 AI 学长，从大一到毕业全程陪伴，帮你搞清楚"去哪、做什么、怎么准备"。

## 功能一览

- **四角色 Agent + 智能路由**（LangGraph）：探索向导 / 实习导航 / 求职教练 / 面试陪练，按年级 + 意图自动切换。
- **RAG 知识库**：内置鹅厂校招 4 篇基础文档，ChromaDB 向量检索，缺依赖时自动降级关键词检索。
- **个性化记忆**：年级 / 专业 / 兴趣 / 近期成长，按画像生成稳定 ID，跨会话连续。
...
- **大三**：「秋招想投鹅厂产品实习，该怎么准备？」→ 路由到 `intern`，岗位匹配 + 鹅厂校招引用 + 三步行动。
...
- 接入腾讯校招官网 RSS / 静态 JSON，让岗位信息按时间窗口动态化。
